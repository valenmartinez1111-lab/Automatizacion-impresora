...
Se corre UNA VEZ (o cada vez que se mueva la camara / cambie el enfoque).

Como se usa:
  1. Cerra Falcon Design Space (la camara no puede estar abierta por dos programas).
  2. Elegi 4 (o mas) puntos de referencia cuya posicion en mm sobre la mesa conozcas
     con precision. Las opciones mas practicas:
       a) Las 4 esquinas de la bandeja/panal de la maquina, si coinciden con el area
          de trabajo (0,0) a (bed_width_mm, bed_height_mm).
       b) Una pieza rectangular de bordes rectos (una madera de sobra, una escuadra)
          apoyada en una posicion que midas con una regla desde el origen (0,0) de
          la mesa, esquina inferior izquierda.
  3. Corre este script. Se abre una ventana con la imagen de la camara.
  4. Click en cada punto de referencia, EN ORDEN, y cuando te lo pida escribi en la
     consola las coordenadas en mm de ese punto (X,Y con el origen en la esquina
     inferior izquierda de la mesa).
  5. Al terminar (minimo 4 puntos, click derecho o tecla 'q' para cerrar), se calcula
     la homografia y se guarda en calibration/camera_calibration.json.

Cuantos mas puntos uses (y mas distribuidos por la mesa), mejor la calibracion.
Se recomienda un minimo de 4 puntos en las esquinas del area util de la camara.
"""
...
def main() -> int:
    ...
    if cal.reprojection_error_mm > 2.0:
        print(
            "ATENCION: el error es alto (>2mm). Repeti la calibracion con mas cuidado "
            "o mas puntos antes de usarla para carteles reales."
        )
    return 0
